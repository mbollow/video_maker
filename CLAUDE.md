...
- **Cover und Hook sind IMMER deckungsgleich** (Nutzer, 30.08.2026: Abweichungen — auch in der Position — stoeren immer). Massgeblich ist `composition_templates/reel-cover.html`: Overlay `rgba(40,29,103,0.5)` ueber dem ganzen Bild, Titelblock `left/right 56px; bottom 300px`, linksbuendig, Montserrat 800/146px mit derselben Auto-Verkleinerung auf die Boxbreite, Pointe in `#4ebbc2`, keine Eyebrow. Dieselben Werte ins Composition-HTML uebernehmen, damit Wortlaut, Umbruch, Position und Schriftgroesse identisch sind. Aendert sich der Cover-Text, aendert sich der Hook mit — nie nur eines von beidem. Weil der Hook damit im unteren Drittel sitzt, ist `HIDE_WINDOWS = [[0, 3.3]]` Pflicht (sonst Kollision mit den Untertiteln). Das Logo oben rechts bleibt im Video, obwohl das Cover keines hat.
...
- **Untertitel-Zeiten gegen Scribe pruefen, bevor ein Reel rausgeht.** Bei Video 013 liefen die aus Whisper gebauten Untertitel dem Ton im Mittel 0,42 s voraus (bis 1,4 s) und enthielten Wortfehler („unverwendliches Informationsbesprich" statt „unverbindliches Informationsgespraech", zweimal im Bild). Juliana meldete das als „Wort schwer verstaendlich" — man liest es, bevor man es hoert. Pruefung: Reel mit `transcribe.py --engine scribe` transkribieren und Cue-Start gegen den Start des ersten Wortes halten; Ziel < 0,1 s. Ein Re-Render der Composition zieht auch Anchors/Listen/Tipps auf die neuen Zeiten mit.
...
- **Caption-Vorprüfung ist Pflicht — VOR dem Upload:** `npm run caption:check` (bzw. automatisch in `ghl_plan.py`) liest die neueste `captions*.txt` jedes FREIGEGEBEN-Ordners gegen. Fundstellen **dem Nutzer nennen, je Stelle einen Korrekturvorschlag, seine Freigabe abwarten** — er entscheidet einzeln. `--execute` bricht ab, solange etwas offen ist; danach mit `--captions-ok` erneut starten. Das Werkzeug findet nur Mechanisches (doppelte Wörter, abgebrochene Sätze, Groß-/Kleinschreibung im Wort) — Sinn und Sachfehler zusätzlich selbst lesen.
- **Karussells in GHL: geklärt (19.08.2026) — die API liest falsch, die Daten sind heil.** Nach dem Terminieren meldet die **API** nur die **erste Folie** (Einzelabruf + Suche, alle Payload-Varianten, auch per UI-Composer terminiert), die **GHL-Oberfläche** zeigt weiterhin alle. Veröffentlicht wird **vollständig und in richtiger Reihenfolge** — belegt auf Instagram und LinkedIn. Es ist also ein reiner Lesefehler der API, kein Datenverlust. **Folge:** Karussells laufen wie jedes andere Format direkt als `scheduled`, kein Draft-Umweg mehr. `npm run ghl:check-media` zählt bei terminierten Mehrbild-Posts entsprechend **nicht** die Folien — die Zahl wäre dort immer 1. Messreihe: `docs/ghl-karussell-mehrbild.md`.
...
- **Scheduling:** Default **09:55 Europe/Berlin**, nächster freier Slot je Format (Reel: Mo+Fr Meta / Fr LinkedIn, Karussell: Mi); Belegung **pro Kanal/Zeitfenster** (Cross-Posting zur selben Zeit ok, ein Kanal nie doppelt). Posts werden **direkt als `scheduled`** angelegt (Default seit 18.08.2026) — die inhaltliche Prüfung passiert vorab im Freigabe-Ordner, der Draft-Umweg kostete nur Klicks im GHL-UI. `--status draft` bzw. `ghl:push:draft` bleibt als Notausgang. Gilt seit dem 19.08.2026 auch für Karussells (siehe nächster Punkt).
- **Archivieren läuft automatisch mit (seit 24.08.2026):** Jeder erfolgreich terminierte Freigabe-Ordner wandert am Ende von `ghl:auto --execute` sofort nach `<Bereich>/veröffentlicht/NNN_slug_<termin>/` — Ordnername = **geplanter Termin**, ältere Versionen (`final_v*`/`v1…v(N-1)`) werden dabei aufgeräumt, Captions/FREIGABE bleiben. Gilt für **alle Formate**. Grund: der tägliche Archiv-Cron hängt am GHL-Feld `publishedAt`, das für Karussells nie gesetzt wird (derselbe API-Lesefehler wie bei den Mehrbild-Folien) — deren Ordner blieben dauerhaft liegen. Ein Ordner bleibt stehen, wenn ein Kanal fehlgeschlagen ist (damit man nachziehen kann) oder bei `--status draft`. Notausgang: `--no-archive`. Der LaunchAgent `com.palstek.freigabe-archive` läuft als Netz weiter.
...
- **Eigennamen: „Juliana Wiechert" (mit *ie*) und „Palstek GmbH".** Scribe/Whisper verhören beides zuverlässig („Wichert", „Wichardt", „Palsteck") — und Kunden reden Juliana im O-Ton mit Namen an, der Name steht also in fast jedem Reel im Untertitel. Bei Video 016 ging „Frau Wichert" eingebrannt raus. Zentrale Tabelle: `helpers/brand_text.py` → `NAMEN`; greift automatisch in Untertiteln (`render.build_master_srt`, `testimonial_build.py`) und Captions (`fix_hashtags`/`fix_post`). Neue Verhörer **dort** ergänzen, nicht pro Projekt; `schreibweisen` in `edl.json`/`testimonial.json` überschreibt sie weiterhin (für Kundennamen).
...
- **LinkedIn-Caption ist ein Denkimpuls, keine Nacherzählung** (Julianas Review 27.08.2026): eigener Einstieg (nie der Video-Hook, auch nicht paraphrasiert), einordnen statt erklären, **die Schritte aus dem Reel nie aufzählen** (sonst verliert das Video seinen Nutzen), dafür ein erweiternder, ruhig unbequemer Gedanke/Reframe. Abschluss ist die **offene Diskussionsfrage**; der Terminlink steht als **letzte Zeile darunter**, sachlich, ohne Werbesatz. Verankert in `brand-guidelines/default/caption-templates/linkedin.md` + `prompts/caption_subagent.md`.
...
- **Sprecher-ID-Box ist Pflicht und läuft automatisch mit:** Kundenlogo + Name + Rolle unten rechts, nur während der Antworten (`idbox`-Block in `testimonial.json`, gebrannt von `testimonial_idbox.py`). Früher ein Handgriff nach dem Build — dabei ist sie bei Rebuilds still verloren gegangen; nach jedem Rebuild einen Talking-Head-Frame prüfen.
...
- **Gewollte Zeilenumbrüche im Innen-Fließtext:** Absätze im `outline.txt` werden normal zu Fließtext verbunden (der Browser bricht nach Breite um). Soll ein Umbruch stehen bleiben (z. B. Formel-Blöcke), beginnt **jede** Zeile des Absatzes mit `|`. Startet eine Zeile danach mit einem Operator (`+ = × > <`) plus Leerzeichen, rutscht der in ein `<span class="op">` fester Breite — die Operatoren stehen untereinander, die Textspalte fluchtet.
