...
- **Karussells nie direkt als `scheduled` anlegen:** die GHL-API behält dabei nur die erste Folie (auf allen Kanälen getestet). Mehrbild-Posts gehen als Draft mit vorbelegtem Termin raus, scharf gestellt wird im GHL-UI. `ghl_plan.py` erzwingt das automatisch.
