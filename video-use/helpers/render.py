...
def main() -> None:
    ...
    # Build master.srt whenever requested — it's just data the composition's
    # subtitle layer consumes. This is decoupled from BURNING subtitles into the
    # video: --no-subtitles skips the burn (so the composition is the only
    # subtitle source) but still writes master.srt if --build-subtitles is set.
