...
Walks <videos_dir> for common video extensions, runs the chosen engine
(Whisper default, Scribe optional) on each, writes transcripts to
<edit_dir>/transcripts/<name>.json.

Cached per-file: any source that already has a transcript is skipped.

Usage:
    python helpers/transcribe_batch.py <videos_dir>
    python helpers/transcribe_batch.py <videos_dir> --engine whisper --workers 6
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="(Scribe only) number of speakers. Improves diarization when known.",
    )
    ap.add_argument(
        "--vocab-prompt",
        type=str,
        default=None,
        help="(Whisper only) vocabulary bias for domain-specific terms.",
    )
    args = ap.parse_args()
    ...
    # Load the appropriate key upfront so we fail-fast if it's missing
